...
### 概要
長期休暇管理機能を使用すると、GW、年末年始、夏季休暇など、複数日にわたる休暇を事前に登録・管理できます。
...
#### 操作手順
1. 管理パネルの「長期休暇管理」タブを開く
2. 「長期休暇の作成 / 更新」セクションで以下を入力：

**必須項目**：
- **対象拠点**：長期休暇を登録する拠点（スーパー管理者の場合のみ選択）
...
1. 「登録済みの長期休暇」一覧から、編集したい項目の**「編集」**ボタンをクリック
2. 入力欄に現在の設定が表示されます
3. 必要な項目を修正します
...
「登録済みの長期休暇」一覧の**「表示」チェックボックス**で切り替えます：

- **チェックON**：
  - ユーザーが長期休暇ボタンから選択・表示できます
  - 複数の長期休暇を同時に表示ONにできますが、ユーザーが選択できるのは1つだけです
  
- **チェックOFF**：
  - 管理者のみ閲覧可能
  - ユーザーには表示されません（長期休暇ボタンにも出ません）
  - 準備中の長期休暇や過去の休暇を非表示にする際に便利です

### ユーザー側の表示

#### 初期状態
- 「表示」がONの長期休暇が1件以上ある場合、ヘッダーに**「長期休暇」**ボタンが表示されます
- 長期休暇がない、または全て非表示の場合はボタンが表示されません

#### ユーザーの操作
1. ユーザーが「長期休暇」ボタンをクリック
2. 表示可能な長期休暇の一覧から選択
3. ガントチャートで休暇状況を確認
...
- ステータス欄に長期休暇のタイトルが表示されます（例：「GW期間休暇」）
...
#### 3. 自動更新
- 長期休暇の追加・編集・削除はサーバーに即座に保存されます
- ユーザーの画面には約30秒以内に反映されます
- すぐに確認してほしい場合は、ユーザーにページの再読み込み（F5）を依頼してください

#### 4. 拠点ごとに管理
- 長期休暇は拠点ごとに独立して管理されます
- 他拠点の長期休暇は表示されません
- スーパー管理者のみ、他拠点の長期休暇を管理できます

#### 5. 期間終了後の扱い
- 長期休暇期間が終了しても、自動的には非表示になりません
- 管理者が手動で「表示」チェックをOFFにするか、削除してください
- 過去の記録として残しておくこともできます

#### 6. ユーザーの編集制限
- ユーザーが長期休暇を表示している間、該当メンバーのステータスは編集できません
...
### Q14. 長期休暇の期間を変更したい
**A**：
- 「登録済みの長期休暇」一覧から該当の休暇の「編集」ボタンをクリック
- 開始日と終了日を変更してください
- 期間を変更すると、ガントチャートが再生成されます
- メンバー指定も再度設定し直す必要がある場合があります

### Q15. 長期休暇を表示しているユーザーがステータスを変更できない
**A**：
- これは正常な動作です
- 長期休暇表示中は、該当メンバーのステータスは「休み」に固定されます
- ユーザーに「表示クリア」を依頼してください
- または、管理者が長期休暇の「表示」をOFFにしてください

### Q16. 長期休暇の表示順序を変更したい
**A**：
- 現在、長期休暇の表示順序を変更する機能はありません
- 登録された順序で表示されます
