...
| **長期休暇** | 長期休暇が登録されている場合に表示。クリックすると長期休暇選択画面が開きます |
| **ログオフ** | ログアウトします |
| **マニュアル** | このマニュアルを表示します |
...
### 概要
長期休暇機能を使用すると、GW、年末年始、夏季休暇など、複数日にわたる休暇を簡単に確認・表示できます。

### 長期休暇ボタンの表示
- 管理者が長期休暇を登録すると、ヘッダーに**「長期休暇」**ボタンが自動的に表示されます
- 長期休暇が登録されていない場合、ボタンは表示されません

### 長期休暇の表示手順

#### 1. 長期休暇モーダルを開く
1. ヘッダーの**「長期休暇」**ボタンをクリック
2. 長期休暇選択画面が表示されます

#### 2. 長期休暇を選択する
1. 登録されている長期休暇の一覧から、表示したいものを選択します
...
2. メイン画面に戻ると、以下のように表示されます：
   ...
   - ステータス欄に長期休暇のタイトル（例：「GW期間休暇」）が表示されます
   - 画面上部に「長期休暇表示中」のバナーが表示されます

#### 4. 長期休暇表示を解除する
- 長期休暇モーダルを開き、**「表示クリア」**ボタンをクリックします
- または、別の長期休暇を選択すると、表示が切り替わります
...
#### 表示中の制限
- 長期休暇を表示している間、該当メンバーのステータスは編集できません
- ステータス欄には長期休暇のタイトルが表示され、内部的には「休み」として扱われます
- 編集が必要な場合は、一度長期休暇表示を解除してください

#### 自動解除について
- 長期休暇期間が終了しても、自動的には解除されません
- 管理者が「表示」設定を解除するか、手動で「表示クリア」を実行してください

#### 長期休暇の登録・編集
- 長期休暇の登録・編集・削除は**管理者のみ**が行えます
- 新しい長期休暇を設定したい場合や、内容を変更したい場合は、管理者に問い合わせてください

### 用途例
長期休暇機能は以下のような場面で活用できます：
...
### Q14. 長期休暇ボタンが表示されません
**A**：長期休暇が登録されていない場合、長期休暇ボタンは表示されません。GWや年末年始などの長期休暇を設定したい場合は、管理者に登録を依頼してください。

### Q15. 長期休暇を表示したのにメンバーの行がハイライトされません
**A**：以下を確認してください：
- 長期休暇モーダルで「表示」ボタンをクリックしたか
- 選択した長期休暇の期間に今日の日付が含まれているか
- 該当メンバーの今日の日付にチェックが入っているか
- ページを再読み込み（F5）して再度試してみてください

### Q16. 長期休暇表示中にステータスを変更できません
**A**：長期休暇を表示している間、該当メンバーのステータスは編集できません。長期休暇モーダルを開き、「表示クリア」ボタンをクリックして解除してください。

### Q17. 長期休暇を設定したい
**A**：長期休暇の登録・編集・削除は管理者のみが行えます。GWや年末年始、夏季休暇などを設定したい場合は、拠点の管理者に問い合わせてください。
