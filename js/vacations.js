(function(){
  ...
  function getDateRange(){
    const startRaw = vacationStartInput?.value || '';
    const endRaw   = vacationEndInput?.value   || '';
  ...
  function updateBitsInput(){
    if(!vacationMembersBitsInput) return;
    vacationMembersBitsInput.value = serializeBits();
  }
  ...
  function rebuild(){
    ...
    parseBitsString(vacationMembersBitsInput?.value || '');
    renderTable();
    bindTableEvents();
  }

  function init(){
    ganttRoot = document.getElementById('vacationGantt');
    if(!ganttRoot) return;
    orderedMembers = getMembersOrdered();
    dateSlots = buildDateSlots();
    parseBitsString(vacationMembersBitsInput?.value || '');
    ...
    if(vacationStartInput){
      vacationStartInput.addEventListener('change', rebuild);
    }
    if(vacationEndInput){
      vacationEndInput.addEventListener('change', rebuild);
    }
    if(vacationMembersBitsInput){
      vacationMembersBitsInput.addEventListener('input', ()=>{
        parseBitsString(vacationMembersBitsInput.value);
        applyBitsToCells();
      });
    }
